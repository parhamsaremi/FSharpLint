...
type ConventionsConfig =
    { recursiveAsyncFunction:EnabledConfig option
      ...
      indexGet: RuleConfig<IndexGet.Config> option
...
with
    member this.Flatten() =
        [|
            ...
            this.indexGet |> Option.bind (constructRuleWithConfig IndexGet.rule) |> Option.toArray
...
let flattenConfig (config:Configuration) =
    ...
    let allRules =
        [|
            ...
            config.IndexGet |> Option.bind (constructRuleWithConfig IndexGet.rule)
