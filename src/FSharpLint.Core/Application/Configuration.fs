...
let flattenConfig (config:Configuration) (onlyEnabled:bool) =
    ...
    let notEnabledConstructRuleWithConfig = forcedConstructRuleWithConfig (not onlyEnabled)

    let allRules =
        [|
            config.TypedItemSpacing |> Option.bind (notEnabledConstructRuleWithConfig TypedItemSpacing.rule)
            ...
            config.PatternMatchClauseIndentation |> Option.bind (notEnabledConstructRuleWithConfig PatternMatchClauseIndentation.rule)
            ...
            config.NestedStatements |> Option.bind (notEnabledConstructRuleWithConfig NestedStatements.rule)
            config.FavourConsistentThis |> Option.bind (notEnabledConstructRuleWithConfig FavourConsistentThis.rule)
            config.CyclomaticComplexity |> Option.bind (notEnabledConstructRuleWithConfig CyclomaticComplexity.rule)
            ...
            config.MaxLinesInLambdaFunction |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInLambdaFunction.rule)
            config.MaxLinesInMatchLambdaFunction |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInMatchLambdaFunction.rule)
            config.MaxLinesInValue |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInValue.rule)
            config.MaxLinesInFunction |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInFunction.rule)
            config.MaxLinesInMember |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInMember.rule)
            config.MaxLinesInConstructor |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInConstructor.rule)
            config.MaxLinesInProperty |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInProperty.rule)
            config.MaxLinesInModule |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInModule.rule)
            config.MaxLinesInRecord |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInRecord.rule)
            config.MaxLinesInEnum |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInEnum.rule)
            config.MaxLinesInUnion |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInUnion.rule)
            config.MaxLinesInClass |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInClass.rule)
            config.InterfaceNames |> Option.bind (notEnabledConstructRuleWithConfig InterfaceNames.rule)
            config.GenericTypesNames |> Option.bind (notEnabledConstructRuleWithConfig GenericTypesNames.rule)
            config.ExceptionNames |> Option.bind (notEnabledConstructRuleWithConfig ExceptionNames.rule)
            config.TypeNames |> Option.bind (notEnabledConstructRuleWithConfig TypeNames.rule)
            config.RecordFieldNames |> Option.bind (notEnabledConstructRuleWithConfig RecordFieldNames.rule)
            config.EnumCasesNames |> Option.bind (notEnabledConstructRuleWithConfig EnumCasesNames.rule)
            config.UnionCasesNames |> Option.bind (notEnabledConstructRuleWithConfig UnionCasesNames.rule)
            config.ModuleNames |> Option.bind (notEnabledConstructRuleWithConfig ModuleNames.rule)
            config.LiteralNames |> Option.bind (notEnabledConstructRuleWithConfig LiteralNames.rule)
            config.NamespaceNames |> Option.bind (notEnabledConstructRuleWithConfig NamespaceNames.rule)
            config.MemberNames |> Option.bind (notEnabledConstructRuleWithConfig MemberNames.rule)
            config.ParameterNames |> Option.bind (notEnabledConstructRuleWithConfig ParameterNames.rule)
            config.MeasureTypeNames |> Option.bind (notEnabledConstructRuleWithConfig MeasureTypeNames.rule)
            config.ActivePatternNames |> Option.bind (notEnabledConstructRuleWithConfig ActivePatternNames.rule)
            config.PublicValuesNames |> Option.bind (notEnabledConstructRuleWithConfig PublicValuesNames.rule)
            config.NonPublicValuesNames |> Option.bind (notEnabledConstructRuleWithConfig PrivateValuesNames.rule)
            config.NonPublicValuesNames |> Option.bind (notEnabledConstructRuleWithConfig InternalValuesNames.rule)
            config.PrivateValuesNames |> Option.bind (notEnabledConstructRuleWithConfig PrivateValuesNames.rule)
            config.InternalValuesNames |> Option.bind (notEnabledConstructRuleWithConfig InternalValuesNames.rule)
            config.MaxNumberOfItemsInTuple |> Option.bind (notEnabledConstructRuleWithConfig MaxNumberOfItemsInTuple.rule)
            config.MaxNumberOfFunctionParameters |> Option.bind (notEnabledConstructRuleWithConfig MaxNumberOfFunctionParameters.rule)
            config.MaxNumberOfMembers |> Option.bind (notEnabledConstructRuleWithConfig MaxNumberOfMembers.rule)
            config.MaxNumberOfBooleanOperatorsInCondition |> Option.bind (notEnabledConstructRuleWithConfig MaxNumberOfBooleanOperatorsInCondition.rule)
            ...
            config.MaxCharactersOnLine |> Option.bind (notEnabledConstructRuleWithConfig MaxCharactersOnLine.rule)
            config.TrailingWhitespaceOnLine |> Option.bind (notEnabledConstructRuleWithConfig TrailingWhitespaceOnLine.rule)
            config.MaxLinesInFile |> Option.bind (notEnabledConstructRuleWithConfig MaxLinesInFile.rule)
            ...
            config.NoPartialFunctions |> Option.bind (notEnabledConstructRuleWithConfig NoPartialFunctions.rule)
        |] |> Array.choose id
