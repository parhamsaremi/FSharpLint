...
let flattenConfig (config:Configuration) (onlyEnabled:bool) =
    ...
    let forcedConstructRuleIfEnabled =
        if not onlyEnabled then
            (fun rule ruleConfig -> Some rule)
        else
            constructRuleIfEnabled

    let forcedConstructRuleWithConfig forced rule ruleConfig =
        if forced then
            ruleConfig.Config |> Option.map rule
        else 
            constructRuleWithConfig rule ruleConfig
    
    let allRules =
        [|
            config.TypedItemSpacing |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) TypedItemSpacing.rule)
            config.TypePrefixing |> Option.bind (forcedConstructRuleIfEnabled TypePrefixing.rule)
            config.UnionDefinitionIndentation |> Option.bind (forcedConstructRuleIfEnabled UnionDefinitionIndentation.rule)
            config.ModuleDeclSpacing |> Option.bind (forcedConstructRuleIfEnabled ModuleDeclSpacing.rule)
            config.ClassMemberSpacing |> Option.bind (forcedConstructRuleIfEnabled ClassMemberSpacing.rule)
            config.TupleCommaSpacing |> Option.bind (forcedConstructRuleIfEnabled TupleCommaSpacing.rule)
            config.TupleIndentation |> Option.bind (forcedConstructRuleIfEnabled TupleIndentation.rule)
            config.TupleParentheses |> Option.bind (forcedConstructRuleIfEnabled TupleParentheses.rule)
            config.PatternMatchClausesOnNewLine |> Option.bind (forcedConstructRuleIfEnabled PatternMatchClausesOnNewLine.rule)
            config.PatternMatchOrClausesOnNewLine |> Option.bind (forcedConstructRuleIfEnabled PatternMatchOrClausesOnNewLine.rule)
            config.PatternMatchClauseIndentation |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) PatternMatchClauseIndentation.rule)
            config.PatternMatchExpressionIndentation |> Option.bind (forcedConstructRuleIfEnabled PatternMatchExpressionIndentation.rule)
            config.RecursiveAsyncFunction |> Option.bind (forcedConstructRuleIfEnabled RecursiveAsyncFunction.rule)
            config.AvoidTooShortNames |> Option.bind (forcedConstructRuleIfEnabled AvoidTooShortNames.rule)
            config.RedundantNewKeyword |> Option.bind (forcedConstructRuleIfEnabled RedundantNewKeyword.rule)
            config.FavourReRaise |> Option.bind (forcedConstructRuleIfEnabled FavourReRaise.rule)
            config.FavourStaticEmptyFields |> Option.bind (forcedConstructRuleIfEnabled FavourStaticEmptyFields.rule)
            config.NestedStatements |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) NestedStatements.rule)
            config.FavourConsistentThis |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) FavourConsistentThis.rule)
            config.CyclomaticComplexity |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) CyclomaticComplexity.rule)
            config.ReimplementsFunction |> Option.bind (forcedConstructRuleIfEnabled ReimplementsFunction.rule)
            config.CanBeReplacedWithComposition |> Option.bind (forcedConstructRuleIfEnabled CanBeReplacedWithComposition.rule)
            config.FailwithBadUsage |> Option.bind (forcedConstructRuleIfEnabled FailwithBadUsage.rule)
            config.RaiseWithSingleArgument |> Option.bind (forcedConstructRuleIfEnabled RaiseWithSingleArgument.rule)
            config.FailwithWithSingleArgument |> Option.bind (forcedConstructRuleIfEnabled FailwithWithSingleArgument.rule)
            config.NullArgWithSingleArgument |> Option.bind (forcedConstructRuleIfEnabled NullArgWithSingleArgument.rule)
            config.InvalidOpWithSingleArgument |> Option.bind (forcedConstructRuleIfEnabled InvalidOpWithSingleArgument.rule)
            config.InvalidArgWithTwoArguments |> Option.bind (forcedConstructRuleIfEnabled InvalidArgWithTwoArguments.rule)
            config.FailwithfWithArgumentsMatchingFormatString |> Option.bind (forcedConstructRuleIfEnabled FailwithfWithArgumentsMatchingFormatString.rule)
            config.MaxLinesInLambdaFunction |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInLambdaFunction.rule)
            config.MaxLinesInMatchLambdaFunction |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInMatchLambdaFunction.rule)
            config.MaxLinesInValue |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInValue.rule)
            config.MaxLinesInFunction |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInFunction.rule)
            config.MaxLinesInMember |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInMember.rule)
            config.MaxLinesInConstructor |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInConstructor.rule)
            config.MaxLinesInProperty |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInProperty.rule)
            config.MaxLinesInModule |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInModule.rule)
            config.MaxLinesInRecord |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInRecord.rule)
            config.MaxLinesInEnum |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInEnum.rule)
            config.MaxLinesInUnion |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInUnion.rule)
            config.MaxLinesInClass |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInClass.rule)
            config.InterfaceNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) InterfaceNames.rule)
            config.GenericTypesNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) GenericTypesNames.rule)
            config.ExceptionNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) ExceptionNames.rule)
            config.TypeNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) TypeNames.rule)
            config.RecordFieldNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) RecordFieldNames.rule)
            config.EnumCasesNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) EnumCasesNames.rule)
            config.UnionCasesNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) UnionCasesNames.rule)
            config.ModuleNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) ModuleNames.rule)
            config.LiteralNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) LiteralNames.rule)
            config.NamespaceNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) NamespaceNames.rule)
            config.MemberNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MemberNames.rule)
            config.ParameterNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) ParameterNames.rule)
            config.MeasureTypeNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MeasureTypeNames.rule)
            config.ActivePatternNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) ActivePatternNames.rule)
            config.PublicValuesNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) PublicValuesNames.rule)
            config.NonPublicValuesNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) PrivateValuesNames.rule)
            config.NonPublicValuesNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) InternalValuesNames.rule)
            config.PrivateValuesNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) PrivateValuesNames.rule)
            config.InternalValuesNames |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) InternalValuesNames.rule)
            config.MaxNumberOfItemsInTuple |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxNumberOfItemsInTuple.rule)
            config.MaxNumberOfFunctionParameters |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxNumberOfFunctionParameters.rule)
            config.MaxNumberOfMembers |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxNumberOfMembers.rule)
            config.MaxNumberOfBooleanOperatorsInCondition |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxNumberOfBooleanOperatorsInCondition.rule)
            config.FavourIgnoreOverLetWild |> Option.bind (forcedConstructRuleIfEnabled FavourIgnoreOverLetWild.rule)
            config.FavourTypedIgnore |> Option.bind (forcedConstructRuleIfEnabled FavourTypedIgnore.rule)
            config.WildcardNamedWithAsPattern |> Option.bind (forcedConstructRuleIfEnabled WildcardNamedWithAsPattern.rule)
            config.UselessBinding |> Option.bind (forcedConstructRuleIfEnabled UselessBinding.rule)
            config.TupleOfWildcards |> Option.bind (forcedConstructRuleIfEnabled TupleOfWildcards.rule)
            config.Indentation |> Option.bind (forcedConstructRuleIfEnabled Indentation.rule)
            config.MaxCharactersOnLine |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxCharactersOnLine.rule)
            config.TrailingWhitespaceOnLine |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) TrailingWhitespaceOnLine.rule)
            config.MaxLinesInFile |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) MaxLinesInFile.rule)
            config.TrailingNewLineInFile |> Option.bind (forcedConstructRuleIfEnabled TrailingNewLineInFile.rule)
            config.NoTabCharacters |> Option.bind (forcedConstructRuleIfEnabled NoTabCharacters.rule)
            config.NoPartialFunctions |> Option.bind (forcedConstructRuleWithConfig (not onlyEnabled) NoPartialFunctions.rule)
        |] |> Array.choose id
